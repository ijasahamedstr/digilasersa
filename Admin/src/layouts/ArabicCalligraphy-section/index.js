...
function ArabicCalligraphy() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setArabicCalligraphy(response.data); // Set the fetched gifts
      } catch (err) {
  ...
  const columns = [
    {
      Header: "ArabicCalligraphy Name",
      accessor: "ArabicCalligraphy_Name",
      width: "20%",
      align: "left",
    },
    { Header: "ArabicCalligraphy Image", accessor: "ArabicCalligraphy_Image", align: "center" },
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_HOST}/ArabicCalligraphy/${id}`);
        setArabicCalligraphy(
          ArabicCalligraphy.filter((ArabicCalligraphy) => ArabicCalligraphy._id !== id)
        );
  ...
  const rows = ArabicCalligraphy.map((item) => ({
    ArabicCalligraphy_Name: (
      <Author
        name={item.arabicCalligraphyname}
        style={{ fontFamily: "Tajawal, sans-serif", fontSize: "18px" }}
      />
    ),
    ArabicCalligraphy_Image: (
      <MDBox>
        {item.arabicCalligraphyimage ? (
          <Image.PreviewGroup>
            <Image
              src={`${process.env.REACT_APP_API_HOST}/uploads/ArabicCalligraphy/${item.arabicCalligraphyimage}`}
              alt="Gift"
    ...
    action: (
      <MDBox display="flex" justifyContent="center" alignItems="center" gap={2}>
        {/* Edit button */}
        <Link
          to={`/Edit-ArabicCalligraphy/${item._id}`}
