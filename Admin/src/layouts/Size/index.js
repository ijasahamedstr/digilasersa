...
function Size() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_HOST}/size`);
        setSize(response.data); // Set the fetched categories
      } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_HOST}/size/${id}`);
        setSize(Size.filter((size) => size._id !== id)); // Update local state after deletion
  ...
  const rows = Size.map((item) => ({
    Size: <Author name={item.size} />,
    action: (
