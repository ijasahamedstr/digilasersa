...
function Partner() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setPartner(response.data); // Set the fetched gifts
      } catch (err) {
  ...
  const columns = [
    { Header: "Partner Name", accessor: "Partner_Name", width: "20%", align: "left" },
    { Header: "Partner Image", accessor: "Partner_Image", align: "center" },
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_HOST}/Partner/${id}`);
        setPartner(Partner.filter((Partner) => Partner._id !== id));
