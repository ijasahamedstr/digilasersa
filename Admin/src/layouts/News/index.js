...
function News() {
  ...
  const columns = [
    { Header: "News Name", accessor: "News_Name", width: "20%", align: "left" },
    { Header: "News description", accessor: "News_dec", width: "20%", align: "left" },
    { Header: "News Image", accessor: "News_Image", align: "center" },
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${process.env.REACT_APP_API_HOST}/News/${id}`);
        setNews(News.filter((newsItem) => newsItem._id !== id)); // Corrected state update
  ...
  // Map through news data to build table rows
