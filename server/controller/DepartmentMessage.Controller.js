...
import Departmentmessage from '../models/departmentmessage.models.js';

// All Account Create
export const Departmentmessagecreate = async (req, res) => {
    const { filename } = req.file;
    const { department, departmentdec } = req.body;

    // Check if all required fields are provided
    if (!lecturername || !lecturerdec || !filename) {
    ...
    try {
        ...
        const userdata = new Departmentmessage({
            lecturername,
            lecturerdec,
            imgpath: filename,
            date
...
// All Acccount View 
export const DepartmentmessageIndex = async (req, res) => {
    try {
        const departmentdecView = await Departmentmessage.find();
...
export const DepartmentmessageDelete = async (req, res) => {
    const departmentDeleteId =  req.params.id;
    
    try {
         await Departmentmessage.deleteOne({_id: departmentDeleteId})
...
export const DepartmentmessageSingleDetails = async (req, res) => {
    try {
        const departmentView = await Departmentmessage.findById(req.params.id);
